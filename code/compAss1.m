% 必做1
% 基于课堂讲授的黑箱建模方法，对上述数据进行预处理后，建立交通流预测模型，
% 以最后两天的数据为预测值，之前的数据为训练值，给出分时段（5分钟，10分钟和15分钟）预测结果，
% 并给出预测精度（平均绝对误差百分比，平均相对误差等指标）。
...
for i=1:datapts-1
    ...
    [c,c0]=linear_regression(Y,X,rerr);
    
    % 预测第15天数据
    X_pre1=flow(1:i,15,preCrs)';
    Y_pre(i+1,1)=(c'*X_pre1'+c0)';
    
    % 预测第16天数据
    X_pre2=flow(1:i,16,preCrs)';
    Y_pre(i+1,2)=(c'*X_pre2'+c0)';
